...
\title[Week 10]{Stat 201: Statistics I\\ Week 10 }
\author[M. Shyne]{}
...
\par}
\medskip
How do should this data be analyzed? It depends on the context of the data or what the data represents.
\begin{itemize}
...
\begin{block}{}
The book uses the following formula for calculating $r$:\\
\medskip
{\centering
$\ds r = \frac{n \Paren{\sum xy} - \Paren{\sum x}\Paren{\sum y}}
...
\medskip
{\centering
$\ds r \propto \sum (x - \bar x) (y - \bar y)$
\par}
\medskip
\pause From this second formula is can be seen that:
\begin{itemize}
...
\end{exampleblock}
\pause{\centering
\includegraphics[width=2in]{../images/ch10_cor_calcr1}\quad
\includegraphics[width=2in]{../images/ch10_cor_calcr2}
\par}
\end{frame}
...
{\centering
\begin{tabular}{c | c | c | c | c }
$u$ & $w$ & $(u - \bar u)$ & $(w-\bar w)$ & $(u - \bar u)(w-\bar w)$\\
\hline
2 & 7 & -3 & 0.5 & -1.5\\
4 & 12 & -1 & 5.5 & -5.5\\
6 & 1 & 1 & -5.5 & -5.5\\
...
\pause\item Make a decision, two ways:
\begin{itemize}
\item P-value: If $p < \alpha$, reject null hypothesis
\item Critical value (from table A-6): If $\Abs t > t_{crit}$, reject null hypothesis
\end{itemize}
\end{itemize}
\end{block}
\end{frame}
...
\begin{block}{}
\large
Predictions for predictor values outside the range of $x$ values used to find the regression line are highly suspect. This is known as \bt{extrapolating} and should be avoid.
\end{block}
