...
\begin{block}{}
\large
One way to think about how hypothesis tests work is to compare them to our legal system:
\begin{itemize}
\item A criminal defendant is ``presumed innocent" until proven guilty. This is equivalent to the null hypothesis. We start with an assumption that there is nothing interesting about our sample.
\item The standard of evidence for criminal trials is ``beyond a reasonable doubt". The significance level ($\alpha$ value) defines what is reasonable doubt in an hypothesis test, how unlikely a sample must be to question the null hypothesis.
\item If there is sufficient evidence, the jury will find a defendant guilty. In other words, if a jury finds that it is unlikely that the evidence that is presented would exist if the defendant was innocent, it rejects the presumption of innocence.    
\end{itemize}
\end{block}
\end{frame}
