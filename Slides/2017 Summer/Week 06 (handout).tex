...
\section{Estimates and Samples Sizes}
...
\begin{alertblock}{}
See ``STATCRUNCH Tutorial for confidence intervals"
\end{alertblock}

\begin{block}{}
\begin{itemize}
\item Stat $\to$ Proportion Stats $\to$ One Sample $\to$ With Summary
\item Enter ``\# of successes" (36) and ``\# of observations" (100)
...
\[ n = \Paren{\frac {z_{\alpha/2} \times s}{ME}}^2 \qquad \text{where} \qquad s = \sqrt{\hat p \hat q}\]
...
\pause\medskip 
Then,
\[n = \Paren{\frac {z_{\alpha/2} \times \sqrt{\hat p \hat q}}{ME}}^2\]
\end{block}
\end{frame}
...
\pause\item $\ds n = \Paren{\frac {z_{\alpha/2} \times \sqrt{\hat p \hat q}}{ME}}^2 = \Paren{\frac {1.96 \times \sqrt{(0.3) (0.7)}}{0.04}}^2 = 504.21 \to 505$ 
\end{itemize}
\pause\medskip
What is wrong with this plan?
\begin{itemize}
\pause\item Retesting, drawing another sample, changes the probabilities of the outcome. The inference is no longer valid.
\end{itemize}
\end{exampleblock}
\end{frame}
...
\begin{alertblock}{}
See ``STATCRUNCH Tutorial for sample size"
\end{alertblock}
...
\item Enter ``Confidence level" if different than 0.95.
\item Enter estimated $\hat p$ as ``Target Proportion" (0.3)
\item Enter twice desired margin of error as ``Width" (0.08)
\item Click ``Compute!"
...
\pause If the population standard deviation $\sigma$ is not known, the sample standard deviation is used and critical values are pulled from \emph{Student's t distribution}.
\end{block}
\end{frame}
...
\begin{block}{}
\begin{itemize}
\item Stat $\to$ Z Stats \bt{or} T Stats $\to$ One Sample $\to$ With Summary
\item Enter ``Sample mean" (66.3), ``Standard deviation" (if known) \bt{or} ``Sample std. dev." (5.79) and ``Sample Size" (40) (not degrees of freedom)
