...
\begin{block}{}
\bt{Classical Approach:} If all simple event are equally likely, then
...
\begin{block}{}
\large The \bt{compement} of event $A$, denoted as $\bar A$, consists of all outcomes in the sample space which are not included in $A$.
\end{block}
...
\vspace*{-\baselineskip}\pause
\item The complement, $P(\text{disease} | \text{positive}) = 0.23$, is known as the \bt{precision} of the test.
\end{itemize}
